...
let print_tab tab = let print_linha k = Array.iter x = (print_char (caracter x) k)
    print_string("\n");
  Array.iter(print_linha tab);;

let puzzle c_linha c_col=
  let alt = length(c_linha) and lg = length(c_col) in  tab = (Array.make_matrix alt lg Empty)
