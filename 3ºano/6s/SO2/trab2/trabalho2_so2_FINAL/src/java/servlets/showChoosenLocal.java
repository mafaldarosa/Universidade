...
public class showChoosenLocal extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Integer> userSpaces = db.getUserSpaces(user.getUserID()); //Apanhas as allergias do user
        
        ArrayList<Entry> entriesBySpace = new ArrayList();
        ...
        for (int i = 0; i < userSpaces.size(); i++) {
            ArrayList<Entry> aux = db.getEntryBySpaceID(userSpaces.get(i));
            for (int j = 0; j < aux.size(); j++) {
                entriesBySpace.add(aux.get(j));
            }
        }
