...
public class showMarkersFromCheckbox extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int size = db.getAllSpace().size(); //numero de allergias que a web app permite
        ...
        for (int i = 0; i < size; i++) {
            if (request.getParameter("Space" + i) != null) {
                String spaces = (String) request.getParameter("Space" + i);
                int spaceID = db.getSpaceId(spaces);
                ArrayList<Entry> entriesAux = db.getEntryBySpaceID(spaceID);
